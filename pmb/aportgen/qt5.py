...
def generate(args, pkgname):
    # Install qt5-qtbase in chroot to get verified apks
    arch = pkgname.split("-")[1]
    pmb.chroot.apk.install(args, ["qt5-qtbase-dev"], "buildroot_" + arch)

    # Parse qt5-qtbase version from APKINDEX
    package_data = pmb.parse.apkindex.package(args, "qt5-qtbase")
    ...
    for subpkgname in ["qt5-qtbase", "qt5-qtbase-dev"]:
    ...
    hashes = pmb.chroot.user(args, ["sha512sum",
                                    "qt5-qtbase-" + version + "-" + arch + ".apk",
                                    "qt5-qtbase-dev-" + version + "-" + arch + ".apk"], "buildroot_" + arch,
    ...
    with open(args.work + "/aportgen/APKBUILD", "w", encoding="utf-8") as handle:
        ...
        handle.write("# Automatically generated aport, do not edit!\n"
                     ...
                     "pkgrel=" + pkgrel + "\n"
                     "arch=\"" + " ".join(arches) + "\"\n"
                     "subpackages=\"qt5-qtbase-dev-" + arch + ":package_dev\"\n"
                     "\n"
                     "_arch=\"" + arch + "\"\n"
        ...
        # Static part
        static = """
            url="https://qt5-qtbase-libc.org"
            license="MIT"
            options="!check !strip"
            pkgdesc="the qt5-qtbase library (lib c) implementation for $_arch"

            _target="$(arch_to_hostspec $_arch)"

            source="
                qt5-qtbase-$pkgver-r$pkgrel-$_arch.apk::$_mirror/edge/main/$_arch/qt5-qtbase-$pkgver-r$pkgrel.apk
                qt5-qtbase-dev-$pkgver-r$pkgrel-$_arch.apk::$_mirror/edge/main/$_arch/qt5-qtbase-dev-$pkgver-r$pkgrel.apk
            "

            package() {
                mkdir -p "$pkgdir/usr/$_target"
                cd "$pkgdir/usr/$_target"
                tar -xf $srcdir/qt5-qtbase-$pkgver-r$pkgrel-$_arch.apk
                rm .PKGINFO .SIGN.*
            }
            package_dev() {
                mkdir -p "$subpkgdir/usr/$_target"
                cd "$subpkgdir/usr/$_target"
                tar -xf $srcdir/qt5-qtbase-dev-$pkgver-r$pkgrel-$_arch.apk
                rm .PKGINFO .SIGN.*
